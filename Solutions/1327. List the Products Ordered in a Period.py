...
def list_products(products: pd.DataFrame, orders: pd.DataFrame) -> pd.DataFrame:
    ...
    filtered_orders = orders[orders['order_date'].dt.year == 2020]
    filtered_orders = filtered_orders[orders['order_date'].dt.month == 2]

    # Merge with products table
    merged = pd.merge(filtered_orders,products,on='product_id')

    # Group by product name and sum units
